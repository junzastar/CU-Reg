...
def processFrame(us_spacing, frame_np, frame_mat, clip_info):
    ...
    # LL-START
    clip_y, clip_x, clip_h, clip_w = clip_info

    fixed_np = frame_np[clip_y:clip_y+clip_h, clip_x:clip_x+clip_w]
    # compute the pixelspacing from the transformation matrix
    mat_scales = computeScale(input_mat=frame_mat)
    # print('matscales {}'.format(mat_scales))
    spacing = np.mean(mat_scales[:2]) / us_spacing[0]
    # print('us_spacing {}'.format(us_spacing))
...
#     descale_mat = np.diag([1 / scales[0], 1 / scales[1], 1 / scales[2], 1])
#     input_mat = np.dot(input_mat, descale_mat)
#     # LL -modified
...
def dof2mat_np(input_dof):
    """ Transfer degrees to euler """
    dof = input_dof

    dof[3:6] = dof[3:6] * (2 * math.pi) / 360.0
    rot_mat = tfms.euler_matrix(dof[3], dof[4], dof[5])[:3, :3]   # SXYZ
...
# continuous rotation parameters to matrix
# first three are translation parameters in voxel, others are two column vectors of rotation matrix
# for single transformation, not for batch
def dof9tomat_np(input_dof):
    # two colume vectors
    a1 = input_dof[3:6]
    a2 = input_dof[6:9]

    # Gram-schmidt orthogonalization to obtain 3D rotation matrix
    # normalize
    ...
    # calculate b3
    b3 = [b1[1]*b2[2]-b1[2]*b2[1], b1[2]*b2[0]-b1[0]*b2[2], b1[0]*b2[1]-b1[1]*b2[0]]
...
def para2Transformation(slice_direction, slice_origin, slice_spacing, parameters):

    mat_S2V = np.identity(4)
    mat_S2V[0:3, 0:3] = torch.tensor(slice_direction).view(3, -1)
    mat_S2V[0:3, 3] = slice_origin
    mat_spacing_slice = np.diag([slice_spacing[0], slice_spacing[1], slice_spacing[2], 1])
    mat_S2V = np.dot(mat_S2V, mat_spacing_slice)
    # physicl to physical transform
    mat_temp = tools.dof2mat_np(parameters)
    mat_S2V = np.dot(mat_temp, mat_S2V)
    return mat_S2V
...
#     # LL modified on 14 June 2023
#     # SXYZ
#     rot_mat = tfms.euler_matrix(dof[3], dof[4], dof[5])[:3, :3]
#
#     mat44 = np.identity(4)
#     mat44[:3, :3] = rot_mat
#     mat44[:3, 3] = dof[:3]
...
#     if scale:
#         scales = dof[6:]
#         mat_scale = np.diag([scales[0], scales[1], scales[2], 1])
#         mat44 = np.dot(mat44, mat_scale)
#
#     # print('mat_scale\n{}'.format(mat_scale))
#     # print('recon mat\n{}'.format(mat44))
#     # sys.exit()
...
def matSitk2Stn(input_mat, clip_size, raw_spacing, frame_shape,
    ...
    # LL-START
    # y for height, x for width
    frame_gt_mat = input_mat
    clip_y, clip_x = clip_size
    corner = np.asarray([clip_x, clip_y, 0])
    trans_mat = np.identity(4)
    # index frame of cropped slice to the index frame of original slice
    trans_mat[:3, 3] = corner
    # slice index to volume index
    frame_gt_mat[:3, 3] -= img_origin
    # index frame of cropped slice to the physical frame of volume
    frame_gt_mat = np.dot(frame_gt_mat, trans_mat)
    # print("frame_gt_mat_before*spacing_mat {}".format(frame_gt_mat))

    # hard to understand
    pos_spacing = np.mean(computeScale(input_mat=frame_gt_mat))
    # print("pos_spacing {}".format(pos_spacing))
    # (0.2493+0.2493+1)/3 = 0.4995
    # the pixel spacing of slice in three directions should be same
    spacing_mat = np.diag([1 / pos_spacing, 1 / pos_spacing, 1 / pos_spacing, 1])
    # create a 4*4 identity matrix
    # physical frame of cropped slice to the physical frame of volume
    frame_gt_mat = np.dot(frame_gt_mat, spacing_mat)
    # print("frame_gt_mat_after*spacing_mat {}".format(frame_gt_mat))
    # print("image_spacing {}".format(img_spacing))
    # image_spacing = (1.0, 1.0, 1.0)
    # print("before_frame_gt_mat[:3, 3] {}".format(frame_gt_mat[:3, 3]))
    # the code below is very hard to understand
    frame_gt_mat[:3, 3] *= [img_spacing[0] / raw_spacing[0],
                            img_spacing[1] / raw_spacing[1],
                            img_spacing[2] / raw_spacing[2]]
    # print("after_frame_gt_mat[:3, 3] {}".format(frame_gt_mat[:3, 3]))
    # the frame_gt_mat is what ???
    ...
    # LL
    # newsp_mat = np.diag([img_spacing[0], img_spacing[1], img_spacing[1], 1])
    # frame_gt_mat = np.dot(frame_gt_mat, newsp_mat)
    # print("hhh {}".format(frame_gt_mat))
    # LL--not suitable due to
    dest_translate = np.identity(4)
    # dest_translate[:3, 3] = np.asarray([frame_shape[1] / 2, frame_shape[0] / 2, 0])
    dest_translate[:3, 3] = np.asarray([frame_shape[1] / 2 * img_spacing[0], frame_shape[0] / 2 * img_spacing[1], 0])
...
# continuous translation and rotation parameters to matrix
# first three are translation parameters in voxel, others are two column vectors of rotation matrix
def dof9tomat_tensor(input_dof, device):
    ...
    # calculate b2
    b1e = b1.unsqueeze(1)   # extend to n*1*3
    a2e = a2.unsqueeze(2)   # extend to n*3*1
    b20 = a2 - torch.bmm(torch.bmm(b1e, a2e), b1e).squeeze(1)
    # nomalize
    b2 = torch.nn.functional.normalize(b20, p=2, dim=1)
    # calculate b3
...
# continuous rotation parameters to matrix
# transltate the two column vectors of rotation matrix into a rotation matrix
def twovolumes2rotationmat_tensor(input_dof):
    # two colume vectors
    a1 = input_dof[:, 0:3]
    a2 = input_dof[:, 3:6]
    ...
    # construct the rotation matrix
    M = torch.zeros((input_dof.shape[0], 3, 3))
    M.requires_grad = True
...
# SXYZ (alpha, beta, gama)
def dof2mat_tensor(input_dof, device, normalize_dof, dof_means=None, dof_std=None):

    
    if normalize_dof:
        # LL
        dof_means = torch.tensor(dof_means).to(device)
        dof_std = torch.tensor(dof_std).to(device)
        output_dof = input_dof * dof_std + dof_means
    ...
    M = torch.zeros((output_dof.shape[0], 4, 4))
...
# LL, eulerangle to rotation matrix
# SXYZ (degree)
def euler2ratationmat_tensor(eulerangle):

    rad = tgm.deg2rad(eulerangle[:, :3])
    ...
    M = torch.zeros((eulerangle.shape[0], 3, 3))
